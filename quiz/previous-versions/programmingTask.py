...
def login():
    #This code allows the user to login to a pre-existing account
    ...
    while userExists == False:
        ...
        with open('userProfiles.csv', 'r') as csvfile: #Opens the file that stores the user data
            ...
            for row in cs:                            #for every row in the file
...
def signup(): #This is ran to setup a user profile
    ...
    while x == True:
        ...
        year = input()
        if age.isnumeric(): #data validation
            x = False
        else:
    ...
    while x == True:
        ...
        rePassword = input()
        if password == rePassword: #data validation
...
def quiz():
    ...
    while topicA == True: #data validation
        topic = input()
        if topic == '1':
    ...
    if topic == 'CompSci': #This is ran if the user wants to do the computer science quiz
            ...
            for row in cs: #The loop for the quiz
                ...
                if diff == '1': #This code is ran if the user wants to do the quiz on the easiest difficulty
                    ...
                    if ansLoc == userAns: #the user answer variable because you can't compare an int to a string
                        ...
                        print("Incorrect")
                elif diff == '2': #This is the code ran for the medium difficulty, refer to the first section as its all the same
                    print(question)
        ...
        with open('his.csv', 'r') as csvfile: #This opens the history quiz file
            cs = csv.reader(csvfile, delimiter = ',') #refer to the first section as its all the same
            for row in cs:
                question = row[0]
                ans = row[1]
                if diff == '1':
